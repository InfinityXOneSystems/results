...
class TripleCheckValidator:
    ...
    def _check_source_authenticity(self, data_point: Dict) -> Dict:
        ...
        is_authentic = any(auth_source in source for auth_source in authoritative_sources)
    ...
    def generate_validation_report(self, data_points: List[Dict]) -> Dict:
        ...
        gold_count = sum(1 for r in results if r["final_status"] == ValidationStatus.GOLD.value)
        silver_count = sum(1 for r in results if r["final_status"] == ValidationStatus.SILVER.value)
        bronze_count = sum(1 for r in results if r["final_status"] == ValidationStatus.BRONZE.value)
        rejected_count = sum(1 for r in results if r["final_status"] == ValidationStatus.REJECTED.value)
    ...
    def get_releasable_data(self) -> List[Dict]:
        ...
        releasable = [
            ...
            if r["final_status"] in [ValidationStatus.GOLD.value, ValidationStatus.SILVER.value]
        ]
        return releasable
...
if __name__ == "__main__":
    ...
    for data in releasable:
        print(f"  - {data['data_point'].get('address', 'unknown')}: {data['final_status']}")
